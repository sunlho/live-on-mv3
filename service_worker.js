...
function keep_two() {
  chrome.windows.getAll({ populate: true }, function (windows) {
    ...
    if (tab_count == 1 && w && w.tabs && !creating_tab) {
      creating_tab = true;
...
function close_one(tab) {
  ...
  chrome.tabs.query({ windowId: tab.windowId }, function (tabs) {
    ...
    for (var i = 0, closed = 0; i < tabs.length && closed < tabs.length - 2; i++) {
      const id = tabs[i].id;
      if (id == tab.id) //Do not close new tab which is chrome://newtab
        continue;
...
chrome.tabs.onActivated.addListener(function (tab) {
  chrome.windows.getAll({ populate: true }, function (windows) {
    ...
    if (tab_count == 2 && w && w.tabs && w.tabs[0].pinned && w.tabs[0].id == tab.tabId && w.tabs[0].url == 'chrome://newtab/') {
      if (!w.tabs[1].id) return;
